...
/*
 * CVodeSetNonlinConvTestFn
 *
 * Specifies the convergence test function in the nonlinear solver convergence
 * test
 */

int CVodeSetNonlinConvTestFn(void *cvode_mem, SUNNonlinSolConvTestFn ConvTestFn)
{
  CVodeMem cv_mem;

  if (cvode_mem==NULL) {
    cvProcessError(NULL, CV_MEM_NULL, "CVODE", "CVodeSetNonlinConvTestFn", MSGCV_NO_MEM);
    return(CV_MEM_NULL);
  }

  cv_mem = (CVodeMem) cvode_mem;

  int flag = SUNNonlinSolSetConvTestFn(cv_mem->NLS, ConvTestFn);
